...
uint256 hashGenesisBlock("0x00000e293381184315af11610ac37c40f9c9e150c82ae0bd06a01a9946c2c0b5");
...
const string strMessageMagic = "Eastercoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
   int64 nSubsidy = 100 * COIN; 
	...
	if(nHeight <= 300) {
                nSubsidy = 1 * COIN; // first 300 blocks 1
        } else if(nHeight <= 4620) {
                nSubsidy = 5000 * COIN;
        } else if(nHeight > 4621) {
                nSubsidy = 0 * COIN;
        } 
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int64 LimUp = nTargetTimespan * 100 / 150; // 150% up
...
*/
        ...
        // Genesis block
        const char* pszTimestamp = "Easter 2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 112;
        block.nTime    = 1397741609;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2739706;
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x55005ecbad992fd79cae723a34542b7205177a4a2258d08992e1bbe7028440f8"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EastercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EastercoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("EastercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("eastercoin-miner");
    ...
    try { loop {
        ...
        printf("Running EastercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("EastercoinMiner terminated\n");
        throw;
    }
}
