...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("ETR")
    updateDisplayUnit();
}
...
{
    QString ret = QString("easter:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ETR unit input in lnReqAmount, we generate the URI with ETR as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ETR, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
